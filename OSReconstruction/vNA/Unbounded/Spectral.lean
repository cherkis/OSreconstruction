...
* `unitaryGroup` - moved to `Unbounded/SpectralPowers.lean`
...
The functional calculus properties in this file follow from the construction:
- Multiplicativity: ∫ fg dP = (∫ f dP)(∫ g dP) (Reed-Simon VIII.5)
- Adjoint: (∫ f dP)* = ∫ f̄ dP (Reed-Simon VIII.5)

The derived power calculus `T^s` and the one-parameter unitary group `U(t) =
e^{itT}` now live in `SpectralPowers.lean`, so the core spectral-construction
file stays below the project line-count guardrail for open theorem files.
